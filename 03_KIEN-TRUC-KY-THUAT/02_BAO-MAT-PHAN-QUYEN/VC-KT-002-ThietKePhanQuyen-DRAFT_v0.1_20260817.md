...
| Permission                                                               | SUPERADMIN | ADMIN | EXPERT | ENTERPRISE | VIEWER |
| ------------------------------------------------------------------------ | ---------- | ----- | ------ | ---------- | ------ |
| `fundingSource.view`                                                   | P          | P⁵    | P⁵    | P⁵         | P⁵    |
| `fundingSource.create` (chỉ ghi chú, không phê duyệt/giải ngân) | P          | O     | —     | O          | —     |

⁵ Quyết định Product Owner (2026-08-20, xem Phụ lục B, B13): mở xem danh mục
toàn nền tảng cho mọi người đã đăng nhập — đúng tinh thần "kết nối" của nền
tảng (giúp tổ chức khác khám phá được nguồn tài trợ, giống cách `need.view`/
`supply.view` dùng `P*` cho bản ghi công khai). Vì `FundingSource` không có
trường trạng thái DRAFT/PUBLISHED nên không cần phân biệt `O`/`P*` như Need/
Supply — toàn bộ bản ghi đã tạo coi như công khai ngay. `create` **không đổi**
— vẫn chỉ `SUPERADMIN`/`ADMIN`/`ENTERPRISE` đúng tổ chức mới được đăng.
...
| B10 | `prisma/schema.prisma` (`User.organizationId` và `ExpertProfile.organizationId`)                                                                                                                                           | Hai trường độc lập, không có ràng buộc đảm bảo luôn khớp nhau — các scope check trong Mục 9 dựa trên`session.user.organizationId`, có thể sai với `ExpertProfile` thật nếu 2 giá trị lệch nhau                                                                                                                                                     | Rủi ro dữ liệu                                      | **Đã vá phần đường code cụ thể gây ra** (2026-08-20, rà cấu phần 01) — xác định được `claimProfileAction`/`decideClaimAction` (`lib/actions/identity.ts`) không kiểm tra người yêu cầu nhận hồ sơ (`claimantUserId`) có cùng tổ chức với `ExpertProfile` hay không; approve claim có thể gán `userId` cho người khác tổ chức → gây đúng tình huống B10 cảnh báo. Thêm kiểm tra 2 lớp: `claimProfileAction` chặn sớm, `decideClaimAction` chặn lại ngay trước khi ghi (phòng thủ theo chiều sâu). Ràng buộc DB cấp schema (VD Prisma check constraint liên bảng) vẫn chưa có — Prisma không hỗ trợ, chỉ enforce được ở tầng action. |
...
| B13 | `app/dashboard/funding-sources/page.tsx` | `db.fundingSource.findMany()` không lọc theo tổ chức/vai trò — mọi người đăng nhập (kể cả `EXPERT`/`VIEWER`) thấy toàn bộ danh mục mọi tổ chức, trong khi Mục 7.4 lúc đó ghi `fundingSource.view = O` (chỉ tổ chức mình) | Tài liệu/code lệch nhau — không phải IDOR ghi, chỉ lệch phạm vi đọc | **Đã xử lý bằng cách sửa tài liệu** — Product Owner xác nhận (2026-08-20) đây là hành vi mong muốn (mở khám phá toàn nền tảng, giống `need.view`/`supply.view`), không phải lỗi code cần vá; Mục 7.4 đã cập nhật `fundingSource.view` từ `O` thành `P` (chú thích ⁵) |
| B14 | `components/matching/match-card.tsx`, `lib/match-labels.ts` | 3/7 giá trị `MatchStage` (`VIEWED`, `CONTACTED`, `COLLABORATING`) có trong schema/enum/nhãn/Match Funnel KPI nhưng không action/nút nào từng gán tới — Match Funnel vĩnh viễn hiển thị 0 cho các cột này dù có dữ liệu thật | Tài liệu/KPI vượt trước mã nguồn | **Đã vá** (2026-08-20) — thêm nút "Đã xem"/"Đã liên hệ"/"Đang hợp tác" nối đúng thứ tự funnel, giữ nguyên các đường tắt cũ; xác nhận qua `AuditLog` (`MATCH_STAGE_VIEWED/CONTACTED/COLLABORATING`) và Match Funnel KPI thật |
| B15 | `components/matching/match-card.tsx` | Nút hành động (Chấp nhận/Từ chối/Tạo dự án) hiện cho **mọi người xem trang**, kể cả `EXPERT`/`VIEWER` không có quyền — trong khi `updateMatchStageAction` chỉ cho `SUPERADMIN/ADMIN/ENTERPRISE` (Mục 7.2). Bấm nút sẽ ném `ForbiddenError` không ai thấy vì không có `try/catch` | Trung bình (UX âm thầm lỗi, không phải IDOR vì action đã tự kiểm tra quyền) | **Đã vá** (2026-08-20) — thêm prop `canManage` (cùng điều kiện `assertOrgScope`/`assertPartyScope`), bọc `try/catch` hiển thị lỗi rõ ràng, khớp cách `GenerateMatchesButton`/`NeedStatusActions`/`ChallengeStatusActions` đã làm |
| B16 | `lib/actions/projects.ts`, `components/projects/projects-list-client.tsx` | `Project.status` tạo xong đứng yên ở `PLANNING` mãi mãi — không action nào từng đổi sang `ACTIVE/ON_HOLD/COMPLETED/TERMINATED`, dù danh sách dự án đã có sẵn thẻ thống kê "Hoàn thành" (`completedCount`) trông chờ dữ liệu này | Tài liệu/KPI vượt trước mã nguồn (cùng dạng B14) | **Đã vá** (2026-08-20) — `updateProjectStatusAction` + `ProjectStatusActions` (dropdown, cùng điều kiện `assertPartyScope`) |
| B17 | `lib/actions/projects.ts`, `components/projects/agreement-panel.tsx` | `Agreement.status` chỉ có `DRAFT → SIGNED`; `COMPLETED`/`TERMINATED` không có nút nào, badge còn tính bằng ternary 2 nhánh nên hiển thị sai màu cho 2 trạng thái này | Tài liệu vượt trước mã nguồn | **Đã vá** (2026-08-20) — `updateAgreementStatusAction`, thêm `AGREEMENT_STATUS_BADGE` đủ 4 trạng thái |
| B18 | `prisma/schema.prisma` (`model Deliverable`), toàn bộ `lib/`/`app/`/`components/` | `Deliverable` có schema đầy đủ và đã được thiết kế quyền ở Mục 7.3 (`deliverable.view`/`deliverable.accept`, scope `PARTY`) nhưng **không một dòng code nào** từng tham chiếu tới model này — không action, không form, không hiển thị | Tài liệu vượt trước mã nguồn hoàn toàn (0% code hoá, không phải lỗi) | **Đã vá** (2026-08-20) — `addDeliverableAction`/`acceptDeliverableAction` (`assertPartyScope`), nộp/nghiệm thu nối trong `MilestonePanel`. `fileUrl` dùng làm đường dẫn ngoài (text nhập tay), chưa có upload file thật như `Need`/`Challenge` — nếu cần, đó là việc kỹ thuật riêng |
...
Đã vá B1-B4, B7, B10 (phần đường code), B11-B20. B11 (đổi tên role) áp dụng trên toàn hệ thống trong
...
mỗi lần vá. B5, B6, B9 và phần `Supply` của B8 còn treo — cần quyết định
nghiệp vụ, không phải lỗ hổng bảo mật hay việc kỹ thuật đơn thuần. B10 đã vá
phần đường code phát hiện được (`claimProfileAction`/`decideClaimAction`);
ràng buộc DB cấp schema (Prisma không hỗ trợ check constraint liên bảng) vẫn
là giới hạn đã biết, không phải việc cần làm ngay.
...
thời cập nhật
`00_QUAN-TRI/VC-QT-003-DanhMucTaiLieu-APPROVED_v1.7_20260820.md`.*
